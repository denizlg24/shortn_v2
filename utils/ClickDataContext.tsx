...
type ClickDataContextType = {
  getClicks: (
    ...
    setClicks: Dispatch<SetStateAction<TClickEntry[]>>,
    setLoading: Dispatch<SetStateAction<boolean>>,
  ) => Promise<void>;
  urlCode: string;
};

const ClickDataContext = createContext<ClickDataContextType | undefined>(
  undefined,
);

export const ClickDataProvider = ({
  children,
  urlCode,
}: {
  children: ReactNode;
  ...
  const getClicks = async (
    ...
    try {
      setLoading(true);
      const response = await fetchApi<{ clicks: TClickEntry[] }>(
        `/clicks/${urlCode}?start=${startDate}&end=${endDate}`,
      );
      if (response.success) {
