...
export async function GET(req: Request, { params }: { params: Promise<{ urlCode: string }> }) {
    ...
    try {
        const startDate = (start && start != "undefined") ? startOfDay(new Date(start)) : undefined;
        const endDate = (end && end != "undefined") ?  endOfDay(new Date(end)) : undefined;
        const clicks = await getClicks(urlCode,startDate,endDate);
        return NextResponse.json({ success: true, clicks:clicks??[] }, { status: 200 });
    } catch (error) {
        console.log(error);
