...
export async function createAndAddTagToQRCode(
  tagName: string,
  qrCodeId: string,
) {
  const session = await getServerSession();
...
export async function addTagToLink(urlCode: string, tagId: string) {
  const session = await getServerSession();
  const user = session?.user;
...
export async function addTagToQRCode(qrCodeId: string, tagId: string) {
  const session = await getServerSession();
  const user = session?.user;
