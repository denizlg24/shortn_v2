...
function FontListItem({
  ...
  const [fontState, setFontState] = React.useState<
      ...
      <div className="flex flex-col gap-1">
        ...
        <span
          className={cn(
            ...
            fontState === "loaded" ? "opacity-100" : "opacity-50",
          )}
          style={{
            fontFamily: fontState === "loaded" ? font.family : "system-ui",
          }}
        >
          {fontState === "error"
            ? "Preview unavailable"
...
export function FontPicker({
  ...
  const displayedFonts = React.useMemo(() => {
    return filteredFonts.slice(0, displayCount);
  }, [filteredFonts, displayCount]);

  const hasMore = displayedFonts.length < filteredFonts.length;
  ...
  // Intersection observer to load more fonts when scrolling
  React.useEffect(() => {
    ...
    observerRef.current = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setDisplayCount((prev) => Math.min(prev + 50, filteredFonts.length));
        }
      },
      { threshold: 0.1 },
    );
  ...
  }, [isOpen, hasMore, filteredFonts.length]);
